  ...
  return (
    <div className="metric-card-glass flex flex-col gap-4 p-5 rounded-[18px] h-full" data-testid="demografico-chart">
      <div>
        ...
        <p className="text-[20px] font-semibold text-[#0c1f3f] dark:text-white">Consumo por edad</p>
      </div>

      <div className="flex flex-col gap-3">
        {GRUPOS.map((g) => (
          <div key={g.label} className="flex items-center gap-3">
            <span className="text-[12px] font-semibold text-[#5580a8] dark:text-white/50 w-10 shrink-0 text-right">
              {g.label}
            </span>
            ...
            <span className="text-[13px] font-bold text-[#0c1f3f] dark:text-white w-10 shrink-0">
              {g.valor}%
            </span>
          </div>
        ))}
      ...
      <p className="text-[11px] text-[#5580a8] dark:text-white/30 mt-auto">
